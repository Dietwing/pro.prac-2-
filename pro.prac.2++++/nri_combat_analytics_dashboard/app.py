...
def sanitize_size_series(series: pd.Series, default: float = 1.0, lower: float = 1.0) -> pd.Series:
    safe = pd.to_numeric(series, errors="coerce").fillna(default)
    return safe.clip(lower=lower)
...
app.layout = html.Div([
    ...
    html.Div([
        dcc.Tabs(id="tabs", value="tab-dashboard", children=[
            ...
            dcc.Tab(label="🛡️ Боевые столкновения", value="tab-encounters"),
...
def render_tab(tab: str):
    ...
    if tab == "tab-encounters":
        return render_encounters_tab()
...
def render_characters_tab():
    characters_view = characters.copy()
    characters_view["initiative_mod_safe"] = sanitize_size_series(characters_view["initiative_mod"])
            ...
            html.Div([
                ...
                dcc.Graph(
                    figure=px.scatter(
                        ...
                        x="armor_class",
                        y="max_hp",
                        size="initiative_mod_safe",
                        color="role",
                        hover_name="character_name",
                        title="Соотношение защиты и здоровья персонажей",
                        labels={"armor_class": "Класс защиты", "max_hp": "Максимальное здоровье", "initiative_mod_safe": "Инициатива"},
        ...
        html.Div([
            ...
            html.Div([
                ...
                dcc.Slider(
                    id="combat-min-round",
                    min=1,
                    max=int(combat_events["round"].max()),
                    step=1,
                    value=1,
                    marks={int(i): str(int(i)) for i in sorted(combat_events["round"].unique())},
                ),
            ], className="filter-item"),
        ], className="filters-row"),
        html.Div([
...
def render_encounters_tab():
    encounters_view = encounters.copy()
    encounters_view["impact_score"] = (
        sanitize_size_series(encounters_view["rounds_planned"], default=1, lower=1)
        * sanitize_size_series(encounters_view["enemy_count"], default=1, lower=1)
    )

    return html.Div([
        html.H2("Справочник боевых столкновений"),
        html.Div([
            html.Div([
                html.H3("Обзор столкновений"),
                dcc.Graph(
                    figure=px.scatter(
                        encounters_view,
                        x="rounds_planned",
                        ...
                        size="impact_score",
                        color="difficulty",
                        hover_name="encounter_name",
                        title="Структура боевых столкновений",
                        labels={
                            "rounds_planned": "Планируемые раунды",
                            "enemy_count": "Количество противников",
                            "impact_score": "Масштаб столкновения",
            ...
            html.Div([
                html.H3("Таблица столкновений"),
                dash_table.DataTable(
                    data=encounters.to_dict("records"),
                    columns=[{"name": col, "id": col} for col in encounters.columns],
                    page_size=10,
                    sort_action="native",
                    filter_action="native",
                    style_table={"overflowX": "auto"},
...
def update_combat(encounter_id: str, min_round: int):
    ...
    df["roll_total_safe"] = sanitize_size_series(df["roll_total"])
    fig = px.scatter(
        df,
        x="round",
        y="damage",
        size="roll_total_safe",
        color="damage_type",
        symbol="hit",
        hover_name="event_label",
        title=f"Хронология столкновения {encounter_id}",
        labels={"round": "Раунд", "damage": "Урон", "roll_total_safe": "Итог броска", "damage_type": "Тип урона", "hit": "Попадание"},
    )
