...
def test_home_page_contains_base_title(dash_server):
    response = requests.get(BASE_URL, timeout=10)
    html = response.text
    assert "НРИ-Помощник" in html
...
def test_dash_layout_contains_main_tabs(dash_server):
    response = requests.get(f"{BASE_URL}/_dash-layout", timeout=10)
    data = response.json()
    layout_str = str(data)
    ...
    for tab_name in [
        ...
        assert tab_name in layout_str, f"Не найдена вкладка: {tab_name}"
